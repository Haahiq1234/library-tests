...
const distance = {
  Vector: {
    ...
    line: function (a, b, p, return_dist = true) {
      ...
      let ab_mag = ab.mag();
      //console.log(ab_mag);
      let am = constraint(Vector.dot(ap, ab) / ab_mag, 0, ab_mag);
...
function getRectSide(rectData, n) {
  switch (n) {
    case 0:
      return [rectData[0], rectData[1], rectData[0] + rectData[2], rectData[1]];
    case 1:
      return [
    ...
    case 3:
      return [rectData[0], rectData[1] + rectData[3], rectData[0], rectData[1]];
  }
}
const Raycast = {
  Vector: {
  ...
  circle(rayOriginX, rayOriginY, rx, ry, cx, cy, circleRadius, n) {
    ...
    let c =
      relativeRayOriginX ** 2 + relativeRayOriginY ** 2 - circleRadius ** 2;
...
class UIElement {
  ...
  calcHoveredInfo() {
    ...
    if (this.shape == UI.CIRCLE) {
      return (
        Vector.dist(Vector.add(position, this.offset), mouse1) <= this.size[0]
      );
    } else if (this.shape == UI.RECT) {
...
class Gizmo extends UIElement {
  ...
  constructor(x, y, col = color(0, 255, 0)) {
    ...
    this.onclick.bind(
      function (gizmo) {
        gizmo.mouseOffset = Vector.sub(gizmo.getMouse(), gizmo.localPosition);
