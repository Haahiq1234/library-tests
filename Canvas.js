...
const distance = {
    Vector: {
        ...
        line: function (a, b, p, return_dist = true) {
            let ab = Vector.sub(b, a);
            let ap = Vector.sub(p, a);
            let ab_mag = ab.mag();
            //console.log(ab_mag);
            let an = Vector.dot(ap, ab) / ab_mag;
            let am = constraint(an, 0, ab_mag);
            //console.log(an, am);
            let m = Vector.AngleToVector(ab.heading(), am).add(a);
            //let m = Vector.add(Vector.setMag(ab, am), a);
            if (return_dist) return Vector.dist(p, m);
            return m;
            // return {
            //   point: m,
            //   dist: Vector.dist(p, m),
            // };
        },
    },
    line: function (ax, ay, bx, by, px, py, return_dist = true) {
        // let abx = bx - ax;
        // let aby = by - ay;
        // let apx = px - ax;
        // let apy = py - ay;
        // let ab_mag = max(abx, aby);
        // let an = abx * apx + aby * apy;
...
const collision = {
    ...
    circleToLine: function (ax, ay, bx, by, cx, cy, cr) {
        let pt = distance.line(ax, ay, bx, by, cx, cy, false);
        let dst = dist(pt.x, pt.y, cx, cy);
...
class RaycastHit {
    ...
    lengthFromXY(x, y) {
        ...
        if (this.hit) {
            this._length =
                ((x - this.point.x) ** 2 + (y - this.point.y) ** 2) ** 0.5;
        } else {
            this._length = Infinity;
        }
        return this._length;
...
const Raycast = {
    ...
    line: function (
        ...
        bx,
        by,
        rs = createVector(rsx, rsy),
        a = createVector(ax, ay),
        b = createVector(bx, by),
    ) {
        // for optimization they can be calculated once and passed in if needed
    ...
    circle(rayOriginX, rayOriginY, rx, ry, cx, cy, circleRadius, n) {
        ...
        let c =
            relativeRayOriginX ** 2 +
            relativeRayOriginY ** 2 -
            circleRadius ** 2;
        let eq = QuadraticFormula(a, b, c).filter(
            (x) => x * rayLength > EPSILON,
        );
        ...
        if (eq.length > 0) {
            ...
            let hit = new RaycastHit(
                true,
                p,
                Vector.sub(p, circleCenter).normalize(),
            );
...
const Rgb = {
    ...
    random: function (...args) {
        if (args.length == 3) {
            return color(
                Random.rangeInt(args[0]),
                Random.rangeInt(args[1]),
                Random.rangeInt(args[2]),
            );
        }
        if (args.length == 6) {
            return color(
    ...
    IndexedLines(verticesArrayXY, indices, fill = false) {
        ...
        for (var i = 0; i < indices.length; i += 2) {
            ...
            if (i > 0 && ai == indices[i - 1]) {
                this.ctx.lineTo(
                    verticesArrayXY[ai * 2],
                    verticesArrayXY[ai * 2 + 1],
                );
            ...
            this.ctx.lineTo(
                verticesArrayXY[bi * 2],
                verticesArrayXY[bi * 2 + 1],
            );
        }
        this.ctx.closePath();
        if (fill) {
...
on.start.bind(function () {
    if (Gizmo.bounds[0] == -1)
        Gizmo.bounds = [
            Gizmo.DEFAULTRADIUS,
            Gizmo.DEFAULTRADIUS,
            CanvasWidth - Gizmo.DEFAULTRADIUS,
            CanvasHeight - Gizmo.DEFAULTRADIUS,
        ];
});
...
class Gizmo extends UIElement {
    ...
    update() {
        ...
        if (this.clicked) {
            ...
            if (this.parent) {
                ...
                this.localPosition.x +=
                    constraint(pos.x, Gizmo.bounds[0], Gizmo.bounds[2]) - pos.x;
                this.localPosition.y +=
                    constraint(pos.y, Gizmo.bounds[1], Gizmo.bounds[3]) - pos.y;
            }
            this.color = this.clickedColor;
        } else if (this.hovered) {
            this.color = this.hoveredColor;
        ...
        if (!this.parent) {
            ...
            //     this.b
            // );
            this.localPosition.x = constraint(
                this.localPosition.x,
                Gizmo.bounds[0],
                Gizmo.bounds[2],
            );
            this.localPosition.y = constraint(
                this.localPosition.y,
                Gizmo.bounds[1],
                Gizmo.bounds[3],
            );
...
class Slider extends UIElement {
    ...
    value(accuracy = 1000) {
        ...
        return (
            Math.round(
                (this.min + (this.max - this.min) * (num / this.len)) *
                    accuracy,
            ) / accuracy
        );
    }
